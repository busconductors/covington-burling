(function () {
  ...
  var currentPreset = 'blank';

  // ── Template Gallery ────────────────────────────────────────────────
  var galleryCards = document.querySelectorAll('.builder-gallery__card');
  galleryCards.forEach(function (card) {
    card.addEventListener('click', function () {
      var preset = this.dataset.preset;
      if (preset) loadPreset(preset);
      highlightGalleryCard(preset);
    });
  });

  function highlightGalleryCard(preset) {
    currentPreset = preset;
    galleryCards.forEach(function (c) {
      c.classList.toggle('builder-gallery__card--active', c.dataset.preset === preset);
    });
    var st = document.getElementById('sumTemplate');
    if (st) {
      st.textContent = preset === 'waiver' ? 'Waiver & Release' : preset === 'nda' ? 'Mutual NDA' : 'Blank Document';
    }
  }
  ...
  function loadPreset(name) {
    ...
    document.getElementById('sendAttachName').textContent = on ? on.value : 'document.pdf';
    renderAll();
    updatePreview();
  }

  // ── Accordion ───────────────────────────────────────────────────────
  var accordionTriggers = document.querySelectorAll('.builder-accordion__trigger');
  accordionTriggers.forEach(function (trigger) {
    trigger.addEventListener('click', function () {
      var panel = this.closest('.builder-accordion__panel');
      ...
      // Open clicked panel (unless it was already open)
      if (!isOpen) {
        this.setAttribute('aria-expanded', 'true');
      }

      // Render clause library content when its panel opens
      if (this.dataset.panel === 'clauseLib' && !isOpen) {
  ...
  function updateAccordionBadges() {
    var fc = document.getElementById('fieldCount');
    var cc = document.getElementById('clauseCount');
    var sc = document.getElementById('sigCount');
    if (fc) fc.textContent = docState.fields.length;
    if (cc) cc.textContent = docState.clauses.length;
    if (sc) sc.textContent = docState.signatureBlocks.length;
  }
  ...
  // ── Update output filename on title change ──────────────────────────
  var docTitleInput = document.getElementById('docTitle');
  if (docTitleInput) {
    docTitleInput.addEventListener('input', function () {
      var on = document.getElementById('outputName');
      if (on && currentPreset === 'blank') {
        var slug = this.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'document';
        on.value = slug + '.pdf';
        document.getElementById('sendAttachName').textContent = on.value;
      }
    });
  }
  ...
  if (outputNameInput) {
    outputNameInput.addEventListener('input', function () {
      document.getElementById('sendAttachName').textContent = this.value || 'document.pdf';
    });
  }
  ...
  function hideSendStatus() {
    if (sendStatus) sendStatus.className = 'send-modal__status';
  }
  ...
  // Autofill name when recipient selected in send modal
  ...
  // Open modal
  document.getElementById('generateSendBtn').addEventListener('click', function () {
    openSendModal();
  });
  ...
  var clauseLib = {
    renderCategories: function () {
      ...
      var searchInput = document.getElementById('clauseLibSearch');
      ...
      container.querySelectorAll('.clause-lib-item').forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          renderAll();
          updatePreview();
          // Open clauses panel and scroll into view
          var clauseTrigger = document.querySelector('[data-panel="clauses"]');
          if (clauseTrigger) clauseTrigger.setAttribute('aria-expanded', 'true');
  ...
  var clauseLibSearch = document.getElementById('clauseLibSearch');
  if (clauseLibSearch) {
    clauseLibSearch.addEventListener('input', function () {
      clauseLib.renderCategories();
    });
  }
  ...
  function restoreDraft(draft) {
    ...
    document.getElementById('sendAttachName').textContent = 'document.pdf';
    highlightGalleryCard('blank');
    renderAll();
    updatePreview();
  }
