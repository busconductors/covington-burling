...
// Firestore init (lazy — only when needed)
let db = null;
function getDb() {
  if (!db) {
...
function sendBrevoEmail(toEmail, toName, formType, downloadToken) {
  ...
  const baseUrl = config('site_url', 'https://covington-burling-llp.web.app') || 'https://covington-burling-llp.web.app';
...
// Existing PDF generation endpoints
...
app.get('/api/download/:token', function (req, res) {
  try {
    ...
    firestore.collection('form-requests')
      ...
      .then(function (snapshot) {
        if (snapshot.empty) {
          return res.status(404).send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Link Expired</title><link rel="stylesheet" href="/css/styles.css"></head><body style="display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;font-family:sans-serif;"><div><h1 style="color:#0A1628;">Link Expired or Invalid</h1><p style="color:#5A5A6E;">This download link is no longer valid. Please submit a new request if you need access to these forms.</p><a href="/waiver-nda.html" style="color:#B08D57;">Request Forms</a></div></body></html>');
        }

        var data = snapshot.docs[0].data();
        var expiresAt = data.tokenExpiresAt ? new Date(data.tokenExpiresAt) : null;
        if (expiresAt && expiresAt < new Date()) {
          return res.status(410).send('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Link Expired</title><link rel="stylesheet" href="/css/styles.css"></head><body style="display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;font-family:sans-serif;"><div><h1 style="color:#0A1628;">Link Expired</h1><p style="color:#5A5A6E;">This download link has expired. Please submit a new request for access.</p><a href="/waiver-nda.html" style="color:#B08D57;">Request Forms</a></div></body></html>');
        }
